...
def write_record(name, details, p_engine):
    ...
    # p_engine.execute(f'INSERT INTO records (name, details) VALUES ({name}, {details})')
...
with column_2:
    ...
    try:
        read_title = st.empty()
        dataset_to_read = st.selectbox('Please select dataset to read',
                                       ([x[0] for x in list_datasets(p_engine_dataset)]))
